...
Date::Date(int y, int m, int d, int h, int min, int s) : year(y), month(m), day(d), hour(h), minute(min), second(s) {
...
std::ostream& operator<<(std::ostream& os, const Date& dt) {
    ...
    os << dt.year << '-' << std::setfill('0') << std::setw(2) << dt.month << '-' << std::setw(2) << dt.day << ' ' << std::setw(2) << dt.hour << ':' << std::setw(2) << dt.minute << ':' << std::setw(2)
       << dt.second;
