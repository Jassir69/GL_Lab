...
#include <unordered_map>              // Include the unordered_map library for using hash maps
...
std::vector<double> SensorManagementServices::calculateMeans(const Sensor &sensor, int period)
{
    ...
    for (const auto &pollutant : pollutants)
    {
        ...
        {                                                                              // If there are measurements for this pollutant
...
std::vector<double> SensorManagementServices::checkSensorOperation(const Sensor &sensorSelected, const std::unordered_map<std::string, Sensor> &allSensors, int period)
{
    ...
    // Find the three closest sensors
    while (nearbySensors.size() < 2)
    {
    ...
    return marginErrors; // Return the vector of margin of error
}
*/
